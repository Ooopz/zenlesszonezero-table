...
/** 驱动盘 2/4 件套效果 HTML（卡片/表格/驱动盘统计悬浮共用） */
...
/** 排序表 HTML（wiki/统计视图共用）：表头三态排序标注由 sort 状态对象（createSort 产物）驱动；
 *  cls 为表格类名（wiki-table/rec-table），className 追加额外类。 */
export function tableHtml(headers, rows, sortable = new Set(), { cls = 'wiki-table', sort = null, className = '' } = {}) {
  const head = headers
    .map((h) => {
