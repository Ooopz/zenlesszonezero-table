// src/web/myChars.js —— 「我的角色」视图渲染（卡片/汇总子页面、行/列拖拽排序）
...
export function characterCard(character) {
  ...
  const addRow = (name, displayFinal, displayBase, displayBonus, highlighted) => {
    ...
    // 灰字 = 理论面板（满级基础+核心技+音擎+盘词条推算），与账号实际值并列定位计算问题；
    // 实测与理论不一致时标红（攻击/生命/防御理论已按游戏规则取整）
    ...
    // 理论值不独占一行：仅实测与推算不一致时同行小字标注；对比信息在悬浮 data-detail
  ...
  // 技能等级：图标 + 等级（悬浮看完整详情；无 skills 时隐藏）
  // 显示顺序：普攻, 闪避, 特殊技, 支援, 大招, 被动
