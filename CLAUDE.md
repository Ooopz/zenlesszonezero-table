...
个人用的《绝区零》角色配装面板：本地 Node 服务器 + 米游社 wiki/账号数据同步 + 无构建步骤的浏览器端 ESM。三个上级视图：「我的角色」（内部「卡片/统计」二级子页面）、「数据库」、「推荐」（内部「驱动盘/音擎/配队/角色数值/工坊配装/工坊音擎/工坊驱动盘/工坊数值」八个子面板）。我的角色展示每个角色的技能等级、影画/潜能觉醒、音擎、驱动盘、最终面板数值与达成率；推荐视图基于推荐方案数据（plans.json）与工坊配装数据（workshop.json / workshop-grad.json）跨角色统计。
...
- **四个数据源**，靠「去标点归一化」的名字匹配合并：
  ...
  4. **工坊配装数据**（`workshop.json` / `workshop-grad.json`，`node src/sync/workshop.js`）：绝区零工坊（`api.zzzmap.com`）的全角色玩家配装实例 + 全服配装统计。配装实例（workshop.json）含音擎/驱动盘/面板（2025 源面板按游戏公式计算），全服统计（workshop-grad.json）是每角色最常用音擎/驱动盘套装，聚合统计（workshop-stats.json）供推荐页「工坊音擎/驱动盘/数值」三个子面板。
...
- **`src/lib/`（双端共享纯模块，Node 与浏览器均可 import）**
  ...
  - `models.js`：领域模型基类 `Character` / `Wengine` / `Disc`。构造时归一化数据、自动算派生属性（如副词条成长次数 `growth`）、组合关系（角色装备音擎+驱动盘）。浏览器把 wiki 与账号数据都实例化成这些基类。`Disc` 保留 `roundIcon`（圆形光盘图标，卡片/统计视图驱动盘用）。
  - `workshopStats.js`：工坊配装数据（`workshop.json` entries）汇总纯函数 `computeWorkshopStats`（音擎/驱动盘按配装条目数聚合、角色面板 P25/P50/P75 分位，百分比统一归一化为小数）。推荐页「工坊音擎/驱动盘/数值」子面板与 `src/sync/workshop.js` 的汇总生成共用。
...
- **`src/sync/`（可执行脚本，也被 server.js 复用导出函数）**
  ...
  - `workshop.js`：工坊数据统一脚本（原 workshop.js/grad/panel/stats/repair 合并）。签名协议（MD5(key+参数排序)）+ 并发池 + 全量配装爬取（`fetchWorkshopData`，同时生成 workshop.json / workshop-grad.json / workshop-stats.json）+ 2025 源面板计算（复现工坊 `enka_attrs_mapping`：角色基础/武器/装备成长公式）+ `--repair` 补面板。依赖 `workshop-static.json`（逆向提取的角色基础/武器/装备/套装静态表）。导出 `apiGet`/`apiPost`/`pool`/`extractBuild`/`fetchWorkshopData`/`fetchWorkshopGrad` 供 server 复用。
- **`src/web/`（浏览器端 ESM，无构建）**
  ...
  - `render.js`：渲染层。「我的角色」视图容器（卡片/统计二级子页面：`myTab`/`setMyTab`/`myCharsShell`/`resolveView`，兼容旧 `card`/`table` 视图值）+ 卡片/统计表格渲染、悬浮提示（`data-detail` 属性 + 全局 mouseover 委托）、行/列拖拽排序、表头点击排序。驱动盘图标优先用圆形光盘（`Disc.roundIcon`，wiki 提取，fallback 账号 icon / library icon）。**内联 `onclick` 引用的函数必须挂到 `window`**（`ui.js` 里注册 `openNote`/`openTargetSettings`）。
  ...
  - `recommend.js`：「推荐」视图容器，八个子面板（驱动盘/音擎/配队/角色数值/工坊配装/工坊音擎/工坊驱动盘/工坊数值），仿 `wiki.js` 的 `TABS` + `PANEL_RENDERERS` 键控分发 + 共享排序（`recSort`/`toggleRecommendSort`）。子面板切换走 `window.ZZZ.recommendTab()`（注册在 `ui.js`）。音擎/配队/角色数值聚合分别在 `src/lib/wengineStats.js`（`computeWengineStats`）/`teamStats.js`（`computeTeamStats`）/`panelRange.js`（`computePanelRanges`，纯函数，可测）；工坊配装（读 `workshop-grad.json`，图表用 `library` 本地图标 + 悬浮套装效果）、工坊音擎/驱动盘/数值（读 `workshop-stats.json` 汇总）渲染在 `recommend.js` 内。**新增推荐子面板 = 上面 `TABS` + `PANEL_RENDERERS` 各加一项**。
...
- eslint 按文件划分全局：`src/web/**`=browser；`server.js`/`src/sync/**`/`src/lib/node.js`/`test/**`=node；`src/lib/util.js`/`schema.js`/`calc.js`=两者（`sort.js`/`workshopStats.js` 纯逻辑无全局，落在基础块）。
- 工坊配装数据：`data/workshop.json`（全角色玩家配装实例，音擎/驱动盘/面板齐全）、`data/workshop-grad.json`（每角色最常用音擎/驱动盘套装）、`data/workshop-stats.json`（音擎/驱动盘按配装条目数、面板 P25/P50/P75 分位）。三者由 `node src/sync/workshop.js` 一步同步生成；server「更新工坊配装」调 `fetchWorkshopData` 同样一步更新。
- 驱动盘圆形图标：`library.discs[name].roundIcon`（wiki 从驱动盘页 modules 提取——出现次数最多的图片即圆形光盘，6 个尺寸变体），「我的角色」卡片/统计视图优先使用；`icon` 为方形套装图。
