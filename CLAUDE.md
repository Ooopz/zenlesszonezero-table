...
- **`src/lib/`（双端共享纯模块，Node 与浏览器均可 import）**
  ...
  - `distStats.js`：分布统计纯函数（`quantile`/`median`/`sd`/`skew`/`kurt`/`pearson`/`computeDist`/`computePowerScore` 综合战力/`kmeans`/`tierFit` 档位匹配）。workshopStats 聚合与 panelBench 共用；**`kmeans` 由 `computeRoleStyles`（流派聚类）实际消费**；computePowerScore/tierFit 仅测试。
