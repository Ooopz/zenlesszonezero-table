...
- **`src/lib/`（双端共享纯模块，Node 与浏览器均可 import）**
  ...
  - `gradStats.js`：`workshop-grad.json`（全服真实占比）聚合纯函数 `computeGradStats`（音擎/单盘套装/套装组合跨角色累加 count 与占比）。「驱动盘」面板（web/discstats.js）消费其 `discs` 作「全服使用」对比列；`wengines` 部分因「音擎」面板已移除不再被统计视图消费（保留模块与测试）。与 `workshopStats.js`（基于排行榜全量样本的真实样本统计）口径不同。
