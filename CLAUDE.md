...
**代码规范与导读**：原 `CODING-STANDARDS.md`（写码强制规范）与 `CODE-GUIDE.md`（代码导读）已于 2026-11 删除——规范要点与代码结构信息以本文档及 git 历史为准（需要时可 `git log --all -- CODING-STANDARDS.md` 找回）。本文档只负责数据口径、设计决策与变更记录。
...
- **workshop-stats.json**：基于 workshop.json（上榜 uid 池）聚合：`panels`（每角色面板真实样本统计：count/min/max/mean/median/sd/IQR/p10-p99/skew/kurt/whisker/outliers/hist）、`wengines`/`discs`（按配装条目数）、`panelCorr`（属性相关，**7 对**：攻击-防御/攻击-生命/防御-生命/暴击率-暴伤/攻击-暴伤/攻击-暴击率/异常精通-异常掌控）、`panelScatter`（暴击率×暴伤、攻击×暴伤 2D 密度，perRole/global）、`discDetails`（30 盘单盘统计）、`relicStats`/`rankDist`/`skillStats`/`roleOwnership`（练度指标；**`rankLayers` 已于 2026-08 从产出移除**——无前端消费却占 348 KB/17.7%，函数与测试已于 2026-11 删除；**`roleDiscStats` 已于 2026-11 随「待定」面板删除**——同无前端消费）、**2026-10 新增**：`roleCooccurrence`（同 uid 同练角色共现 → 真实配队亲和）、`roleStyles`（**角色流派分析**：面板 k-means 聚类 k=3，**聚类属性池按角色定位（trait）选**——击破含冲击力、异常含精通/掌控、命破/防护含生命防御、支援含攻击生命，由 buildWorkshopStats 经 roleNameMap+library 构造 traits 传入，无定位回退通用池；再数据驱动去噪（cv<0.04 的列剔除，至少保留 3 维），每角色 {attrs, styles:[{share,label,panel(mean/median),main{4,5,6} Top2,suits Top2,wengine Top2}]}，4 号位主词条是强判别信号；⚠️ 暴伤等百分比聚合值为小数（mys "165.2%"→1.652），前端按值 ≤3 判百分比显示）、`weightJson`（工坊权重）、**2026-08 新增**：`rollEfficiency`（每角色加权词条效率分 + 槽位短板 + D9 评分×毕业度相关）。**口径：画像按玩家真实样本统计，不当作全服分布**。
- **浏览器端只加载 workshop-stats.json（≤2 MB）**；一切新聚合在同步脚本完成（流式遍历 358k+ 条，一次 ~2-4 分钟）。⚠️ 2026-08 加入 rollEfficiency/slotDist 后实测 **1.82 MB**，余量已不多——再加聚合前先量体积，必要时对 role 级明细做 Top-N 截断。
...
- **两源判别 `sourceOf(e)`**：`source` 字段 → **`equips[].rarity` 的类型**（string `"S"`=mys / number `4`=2025）→ skills 数组顺序，逐级兜底。rarity 判源实测 20 万条目可判率 100%、与 `subs[].value` 形态（mys 字符串 / 2025 数字）100% 同构零交叉；旧的数组顺序启发式与它分歧 160 条（0.080%），分歧样本全是 rarity 全 `"S"`（mys）但 skills 恰好呈 ID 升序 `[0,1,2,3,5,6]`——**误判的是数组顺序法**，故降为最后兜底。技能语义归一（`computeSkillStats`）共用此判别，口径必须一致。
...
- **`src/lib/`（双端共享纯模块，Node 与浏览器均可 import）**
  ...
  - `panelBench.js`：推荐方案三档统计纯函数 `computeRecTierStats`（plans → 每角色每属性 low/mid/high 的 mean/median/sd/cv，MAD 排除离群哨兵值，过滤 low=mid=0 占位属性）。供统计视图（推荐三档图/共识度）消费，结果有缓存（`recommend.js` 的 `recTierStats`）。
...
- **`src/web/`（浏览器端 ESM，无构建）**
  ...
  - `charts.js`：ECharts 图表辅助（依赖 index.html 引入的本地 vendor `src/vendor/echarts.min.js`（5.5.0），`window.echarts`）。主题色 `CHART_COLORS`（匹配项目暗色+金色）+ 半透明变体 `SOFT` + 公共片段（`AXIS_LINE`/`AXIS_LABEL`/`AXIS_LABEL_SMALL`/`SPLIT_LINE`/`CHART_LEGEND`/`CHART_TITLE`/`CHART_SUBTITLE`/`DARK_TOOLTIP`）——**所有图表统一引用这些基座，禁止硬编码色值**；`registerChart`/`clearCharts`/`mountCharts`/`chartBox` 渲染挂载机制（**页面 resize 自动重排已挂载图表**，防抖 150ms）+ 各图表 option 构建函数（共识度大图（每属性子图）/小提琴箱线/2D 密度散点/**`tierRichOption`（推荐三档 × 玩家分布：每属性 4 行——玩家 P10-P90 / 三档 median±sd 用 markArea 区间、我的值金色竖线+百分位；x 轴显式开启数值轴 axisPointer 竖线（默认样式，与技能分布图一致）+ 透明辅助系列提供悬浮数值锚点）**/**练度图**：`rankPyramidOption`（影画金字塔堆叠，0 影深青 1 蓝灰 2 蓝 3 绿 4 金 5 橙 6 红）、`relicBarOption`（装配评分箱线）、`skillDistOption`（技能等级分布子图，我的等级柱高亮）、**`scoreRelicOption`（D9 评分×毕业度：每角色 relic_point × 加权效率分皮尔逊 r 横向条，绿 ≥0.90 / 金 ≥0.80 / 橙 <0.80；**按 r 降序传入**——类目轴首项画在底部，最脱节的角色才落在图顶）**/**驱动盘图**：`discMain456Option`（456 堆叠）/`discSubsOption`/`discComboOption`/`mainSubCrossOption`（主词条×副词条协同热力，4/5/6 槽并排，色=条件频率）**（达标热力 `heatmapOption` 已于 2026-11 随「待定」面板删除；D7 全盘「套装 × 槽位交叉」热力图 `discSlotHeatOption` 已于 2026-10 删除）/读数参考线 `attachReadLine`（option 带 readLine 标记启用：小提琴图鼠标在子图内任意位置显示灰色横虚线 + y 轴数值标签，空白处 showTip 显示对应密度区间，`graphic` + 原生 DOM mousemove 实现）**。视觉统一项目主题。
  - `recommend.js`：「统计」视图容器，**三个**子面板（角色面板/驱动盘/全服总览），仿 `wiki.js` 的 `TABS` + `PANEL_RENDERERS` 键控分发 + 共享排序（`recSort`/`toggleRecommendSort`）。子面板切换走 `window.ZZZ.recommendTab()`（注册在 `ui.js`）。「全服总览」（原「角色总览」+「练度总览」合并）：**共识度散点大图**（每属性一子图：玩家分化 sd × 攻略分歧 CV）+ **评分 × 盘毕业度**（D9：每角色 `rollEfficiency[role].scoreVsRelic` 的 r 横向条，r 高=工坊评分可当毕业度代理、r 低=该角色须改看有效强化次数）+ `progressCardsHtml()`（装配评分分布箱线/影画档位金字塔/面板属性相关表（固定 7 列，不包滚动容器直接铺开））；「角色面板」（**流派分析卡**（2026-10：玩家池面板 k-means 流派 + 占比堆叠条 + 典型面板表（mean/中位）+ 456 主词条/套装/音擎偏好 + 我的联动「你的面板最贴近 XX 流」）+ 玩家分布箱线（小提琴+箱线，叠加推荐三档点 + 我的）+ 推荐三档×玩家区间增强图 + 面板属性对密度散点 + **技能对标分布图**（玩家池分布子图 + 我的等级金色柱，官方 type 经 `OFFICIAL_SKILL_TYPE` 映射到 canonical） + **角色配装对标卡片**（该角色工坊实况 vs 方案推荐：`plansStats.computeRoleBuildsFromPlans` 方案侧 Top 音擎/套装（组合名经 `orderComboSets4First` 归一 4 件套在前）与 workshop-grad 实况并排 + 差异分析） + **配队亲和卡**（玩家实配队友（roleCooccurrence 同练共现）vs 攻略配队（plans team 同队成员）两口径 Top6 对比，B6），角色下拉走 `ZZZ.selectRole`）为 ECharts 图表面板，render 后由 `mountRecommendCharts` 挂载（render.js 调用）。role_id 键的 stats（relicStats/rankDist/skillStats/roleCooccurrence/rollEfficiency）经 `roleKeyedMap` 统一映射到 plans 角色名（grad 名对齐）。**所有图表卡标题用短名，详细说明放 `data-detail` 悬浮**（无原生 title 小框）。**新增统计子面板 = 上面 `TABS` + `PANEL_RENDERERS` 各加一项**。
...
| D4 有效词条分布（毕业度）      | ✅ **口径已改为「有效强化次数」**（旧「有效词条个数」99.95% 恒为 4，无区分度）：决策卡实况组合 Top + effDist（「我的盘 vs effDist 百分位」的毕业度矩阵已随「待定」面板于 2026-11 删除）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| D8 角色级配装画像              | ❌ `roleDiscStats` 聚合与「角色级配装画像」消费已于 2026-11 随「待定」面板删除（无前端消费方）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| D10 两源一致性审计             | ❌ 已随「待定」面板于 2026-11 删除——`sourceAudit` 聚合、`computeSourceAudit` 与审计表一并移除（判源判别 `sourceOf` 仍保留，供技能归一等使用）。原基线备忘：**2026-08 首跑** 83 万条目判源覆盖率 100%（mys 38.1 万 / 2025 44.9 万），342 个「角色×属性」对 \|diff\| 中位 1.00%（红 9 格 / 橙 25 格 / 绿 308 格）；唯一显著异常为**本·比格攻击力 -41.5%**（核心被动把防御转攻击，mys 含转化、2025 复算公式不含角色专属被动——已知边界非数据损坏） |
...
- 工坊配装数据：`data/workshop.json`（分块 gzip 非固实压缩：第 0 行 `{meta, perChunk, offsets}` 索引头 + 每块独立 gzip 的 JSON 数组，2.08GB → 0.16GB、全量读取 ~16s、聚合 ~90s；旧格式由 `scripts/convert-workshop-gz.mjs` 转换，硬链接备份 `.bak-json`）、`data/workshop-grad.json`（57 角色最常用音擎/驱动盘套装）、`data/workshop-stats.json`（聚合，无 abyssStats/无 playerProfiles）、`data/workshop-weights.json`（角色默认流派权重）。全部由 `node src/sync/workshop.js` 一步同步生成；server「更新工坊配装」调 `fetchWorkshopData` 同样一步更新。**口径：workshop-grad 是全服真实累计占比（工坊 `grad_stat` 接口）；workshop-stats 基于 workshop.json（上榜 uid 池的完整角色池）聚合，画像按玩家真实样本统计 min/max/mean/median，不当作全服分布。**
...
- **加权效率分卡片**：`rollEfficiency` 目前只被 D9 卡用来取 `scoreVsRelic`（毕业度矩阵曾用它取「有效副词条集合」，已随「待定」面板于 2026-11 删除），尚未渲染「我的分 vs 玩家池分布百分位 + 短板槽（slotEff）」这张最有价值的卡。实测 `slotEff` 已有明确结构：1/2/3 槽有效强化次数均值 5.1-5.9，4/5/6 槽仅 4.4-4.8（主词条槽挤占副词条空间）。

## 变更记录（2026-09 ~ 2026-10）

- **2026-11（删除「待定」统计子面板）**：统计视图第四子面板「待定」整体删除——提升清单/面板达标（达标热力）/驱动盘毕业度矩阵/两源一致性审计 四卡及渲染函数（`renderPending`/`upgradeAndHeat`/`discMatrixCard`/`auditCard`）、达标热力图 `heatmapOption`、聚合 `roleDiscStats`/`sourceAudit`（`computeRoleDiscStats`/`computeSourceAudit` 及其测试）一并清理；`workshop-stats.json` 重算去死载荷。统计视图恢复为三个子面板（角色面板/驱动盘/全服总览）。
- **2026-11（滚动体系 / 子 tab 持久化 / 卡片收窄 / 删文档）**：① 滚动体系改 **body 级滚动**——`main`/`.wiki-wrap`/`.tbl-wrap`/`.coverage-table-wrap`/`.style-table-wrap` 去掉纵向限高内滚与隐藏滚动条（`max-height`+`overflow:auto`+`scrollbar-width:none` 删除），表格纵向完整展开、页面整体滚动；宽表格保留 `overflow-x:auto` 横向滚动兜底；吸顶表头拆两套：弹窗内表格（`.plantable`/`.skill-table`，相对 `.modal-scroll`）`top:0`，主区域表格（`.tbl`/`.wiki-table`/`.rec-table`，随 body 滚动）`top:var(--head-h)` 让开 sticky header；② **子 tab 持久化**——`?view=&tab=&role=&disc=` 用 `history.replaceState` 写入，切换视图/子 tab/角色下拉/盘下拉即同步，`initUi` 时从 URL 恢复（`ui.js` 的 `syncUrl`/`applyUrlState`/`URL_TABS`；`myTab`/`selectedRole`/`selectedDisc` 改为 `export let` 供 URL 读取，legacy `card`/`table`/`discstats` 视图值兼容）；③ 卡片/图表网格基准宽度 **640→560px**（`main`/`.mychars-body.cards`/`.chart-grid`）；④ **删除 `CODE-GUIDE.md` 与 `CODING-STANDARDS.md`**（规范与导读以本文档及 git 历史为准）。
...
- **2026-10（统计面板重组「待定」）**：统计视图新增第四子面板「待定」（标签名待定，随时可改），全服总览的 提升清单/面板达标（达标热力）/驱动盘毕业度/两源一致性审计 四张卡移入；全服总览只保留 共识度散点/影画×评分/评分×盘毕业度/练度总览（影画×评分已于 2026-11 删除）。重构：四卡构建抽为模块顶层函数 `upgradeAndHeat`/`discMatrixCard`/`auditCard`，新面板渲染走 `renderPending`（TABS/PANEL_RENDERERS 各加一项）；达标热力 chartBox key 改 `pending-heat`。（该面板已整体删除，见 2026-11 变更记录）
...
- **2026-10（第二批可视化）**：新增 4 聚合（roleCooccurrence/completeness/rankRelic/skillCombos）+ 9 张新卡/图——全服总览：提升清单（缺口×落后度排序）、达标热力图按落后度重排+缺口悬浮、驱动盘毕业度矩阵升级（主词条主流对照+替换建议）、完成度矩阵、影画×装配评分条；角色面板：技能组合卡（拉满模式+我的对照）、配队亲和卡（玩家实配 vs 攻略配队两口径）；驱动盘面板：主词条×副词条协同热力图、组合画像（双暴/攻击+双暴）。**（completeness / 完成度矩阵已于 2026-08 整改中删除，见上；技能组合卡与 skillCombos 聚合已于 2026-11 删除；影画×装配评分卡与 rankRelic 聚合已于 2026-11 删除；提升清单/达标热力图/驱动盘毕业度矩阵随「待定」面板于 2026-11 删除）**
...
- **2026-08**：工坊爬取口径改为排行榜全量 uid（高练度标杆池）；技能练度/驱动盘两源同构提取；练度指标聚合（relicStats/rankLayers/rankDist/skillStats/roleDiscStats）；驱动盘决策卡；统计视图图表面板体系成型。（`roleDiscStats` 已随「待定」面板于 2026-11 删除）
...
- **2026-11（workshop.json 分块 gzip 非固实压缩）**：内容仍是普通 JSON（entries 数组），按每 2 万条切成一块、每块独立 gzip（非固实：任一块可独立解压/定位），第 0 行为 {meta, perChunk, offsets} 索引头。实测 829,891 条：2.08GB → 0.16GB（-92%），全量读取 ~16s（原逐字符流式解析 ~60s+），全量聚合 ~90s（原 2-4 分钟）；与旧文件聚合结果逐位一致（前 10 万条 wengines/discs/panels）。
  - 存储层在 lib/node.js：readWorkshopHeader（索引头）/ iterWorkshopFile（逐块解压整块 JSON.parse，流式 yield）/ writeWorkshopFile（meta 可为函数按实际条数生成，原子 tmp+rename）/ readLines（PART 行读取）；
  - 写入端 flushPart 改逐行完整 JSON（PART 临时文件），mergeWorkshopFile 改为「旧文件逐块解码 + PART 逐行解码 → 重新分块压缩」；
  - clean-workshop.mjs 重写为逐块解压清洗再重压；rebuild-stats.mjs 头部读取走 readWorkshopHeader；
  - 转换 scripts/convert-workshop-gz.mjs（幂等 + 硬链接备份 .bak-json，80.5s 完成）；新增 test/workshop-gz.test.js（多块往返/空文件/单块独立解压）并重写 workshop-merge.test.js；
  - 179 单测全绿 + lint 干净。

- **2026-11（角色面板删除技能组合卡）**：统计 → 角色面板的「技能组合」卡删除（B9，玩家拉满模式 Top + 我的对照）——computeSkillComboStats/makeSkillComboStatsAcc 连同 computeAllWorkshopStats 接线一并移除（唯一消费方即该卡，无其他用途；聚合 15 项 → 14 项），前端 skillComboCardsHtml 删除，测试同步删（含逐位相等接线）。已落盘的 workshop-stats.json 里旧 skillCombos 字段不再被读取，下次同步或 
pm run rebuild:stats 后自然消失。

- **2026-11（全服总览删除影画 × 装配评分卡）**：统计 → 全服总览的「影画 × 装配评分」横向条删除（每角色 6影 median − 0影 median，rankRelic）——`computeRankRelic`/`makeRankRelicAcc` 连同 computeAllWorkshopStats 接线一并移除（唯一消费方即该卡；聚合 14 项 → 13 项），前端 `rankRelicGapOption`（charts.js）与调用处删除，测试同步删（含逐位相等接线）。已落盘的 workshop-stats.json 里旧 `rankRelic` 字段不再被读取，下次同步或 `npm run rebuild:stats` 后自然消失。
